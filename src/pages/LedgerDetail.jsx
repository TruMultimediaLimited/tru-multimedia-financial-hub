    ...
    <div>
      ...
      <div className="border border-gray-800 rounded-lg p-4 mb-4">
        <div className="flex items-start justify-between mb-2">
          <div>
            <div className="text-lg font-semibold text-gray-100">{party}</div>
        ...
        {transaction.description && <p className="text-sm text-gray-400 mb-3">{transaction.description}</p>}

        <div className="grid grid-cols-3 gap-3 text-sm mb-3">
          <div>
            <div className="text-xs text-gray-500">Total</div>
            <div className="text-gray-100">{formatMoney(transaction.total_amount)}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500">Paid</div>
            <div className="text-gray-100">{formatMoney(paidAmount)}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500">Due</div>
            <div className={dueAmount > 0 ? 'text-due' : 'text-gray-100'}>{formatMoney(dueAmount)}</div>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-3 py-1.5 rounded-md text-xs border border-gray-700 text-gray-300"
          >
            Edit
          </button>
      ...
      <h2 className="text-sm font-medium text-gray-300 mb-2">Payments</h2>
      ...
      <h2 className="text-sm font-medium text-gray-300 mb-2">Add payment</h2>
      <PaymentForm
      ...
      <div className="border border-gray-800 rounded-lg p-3 space-y-2">
        ...
        <div className="flex gap-2">
          <button
            onClick={handleSave}
            disabled={saving}
            className="px-3 py-1.5 rounded-md text-xs bg-gray-100 text-gray-900 disabled:opacity-50"
          >
            {saving ? 'Saving…' : 'Save'}
          </button>
          <button onClick={() => setEditing(false)} className="px-3 py-1.5 rounded-md text-xs border border-gray-700 text-gray-300">
            Cancel
          </button>
        </div>
      </div>
