    ...
    <div>
      ...
      <div className="bg-surfaceRaised border border-gray-200 rounded-lg p-4 mb-4">
        <div className="flex items-start justify-between mb-2">
          <div>
            <div className="text-lg font-semibold text-gray-900">{owner.name}</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
          ...
          <div>
            ...
            <div className={owner.netOwedToOwner !== 0 ? 'text-due' : 'text-gray-900'}>
              {formatMoney(Math.abs(owner.netOwedToOwner))}
            </div>
          </div>
          <div>
            <div className="text-xs text-gray-500">Invested</div>
            <div className="text-gray-900">{formatMoney(owner.totalInvested)}</div>
          </div>
        </div>
      </div>
      ...
      <h2 className="text-sm font-medium text-gray-700 mb-2">Investment history</h2>
      ...
      <h2 className="text-sm font-medium text-gray-700 mb-2">Money received (collected on company's behalf)</h2>
      ...
      <h2 className="text-sm font-medium text-gray-700 mb-2">Money given (paid company expenses out of pocket)</h2>
      <div className="space-y-2 mb-4">
...
function PaymentHistoryRow({ payment, onClick }) {
  return (
    <div onClick={onClick} className="flex items-center justify-between border border-gray-200 rounded-lg p-3 cursor-pointer hover:bg-surfaceRaised/60">
      <div>
        <div className="text-sm text-gray-900">{payment.transactions?.category || 'Uncategorized'}</div>
        <div className="text-xs text-gray-500">
          {payment.transactions?.concerns?.name} · {formatDate(payment.payment_date)} · via {CHANNEL_LABELS[payment.channel]}
          {payment.note && ` · ${payment.note}`}
        </div>
      </div>
      <div className="text-sm text-gray-900">{formatMoney(payment.amount)}</div>
    </div>
  );
}
