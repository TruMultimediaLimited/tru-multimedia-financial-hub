...
export default function OwnerDetail() {
  ...
  return (
    <div>
      <button onClick={() => navigate('/owners')} className="text-xs text-gray-500 mb-3">
        ← Back to Owners
      </button>
      ...
      <div className="border border-gray-800 rounded-lg p-4 mb-4">
        <div className="flex items-start justify-between mb-2">
          <div>
            <div className="text-lg font-semibold text-gray-100">{owner.name}</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
          ...
          <div>
            <div className="text-xs text-gray-500">{owner.netOwedToOwner >= 0 ? 'Company owes' : 'Owner owes company'}</div>
            <div className={owner.netOwedToOwner !== 0 ? 'text-due' : 'text-gray-100'}>
              {formatMoney(Math.abs(owner.netOwedToOwner))}
            </div>
          </div>
          <div>
            <div className="text-xs text-gray-500">Invested</div>
            <div className="text-gray-100">{formatMoney(owner.totalInvested)}</div>
          </div>
        </div>
      </div>
      ...
      <h2 className="text-sm font-medium text-gray-300 mb-2">Investment history</h2>
      <div className="space-y-2 mb-3">
        {investments.length === 0 && <p className="text-sm text-gray-500">No investments recorded yet.</p>}
        {investments.map((inv) => (
          <div key={inv.id} className="flex items-center justify-between border border-gray-800 rounded-lg p-3">
            <div>
      ...
      <h2 className="text-sm font-medium text-gray-300 mb-2">Money received (collected on company's behalf)</h2>
      <div className="space-y-2 mb-4">
        {received.length === 0 && <p className="text-sm text-gray-500">Nothing recorded yet.</p>}
        {received.map((p) => (
          <PaymentHistoryRow key={p.id} payment={p} onClick={() => navigate(`/ledger/${p.transaction_id}`)} />
        ))}
      </div>

      <h2 className="text-sm font-medium text-gray-300 mb-2">Money given (paid company expenses out of pocket)</h2>
      <div className="space-y-2 mb-4">
        {given.length === 0 && <p className="text-sm text-gray-500">Nothing recorded yet.</p>}
        {given.map((p) => (
          <PaymentHistoryRow key={p.id} payment={p} onClick={() => navigate(`/ledger/${p.transaction_id}`)} />
        ))}
      </div>
...
function PaymentHistoryRow({ payment, onClick }) {
  return (
    <div onClick={onClick} className="flex items-center justify-between border border-gray-800 rounded-lg p-3 cursor-pointer hover:bg-surfaceRaised/60">
      <div>
        <div className="text-sm text-gray-100">{payment.transactions?.category || 'Uncategorized'}</div>
        <div className="text-xs text-gray-500">
          {payment.transactions?.concerns?.name} · {formatDate(payment.payment_date)} · via {CHANNEL_LABELS[payment.channel]}
          {payment.note && ` · ${payment.note}`}
        </div>
      </div>
      <div className="text-sm text-gray-100">{formatMoney(payment.amount)}</div>
    </div>
  );
}
